...
Do not use Rules-of-Thumb, gold labels, hidden explanations, or external memories.
...
def build_interaction_memory(
    ...
    print(f"\n[Adaptation: no RoT] {partner_name} <-> korea | n_turns={args.n_turns}")
    adaptation_history = no_rot.run_adaptation_dialogue_no_rot(
        tokenizer,
        model,
        partner_system_prompt,
        base.exp.PERSONAS["korea"],
    ...
    debate_transcript = no_rot.build_debate_transcript_no_rot(
        adaptation_history,
        partner_name,
        "korea",
    )
        ...
        json.dump(
            {
                ...
                "condition": condition,
                "interaction_uses_rule_of_thumb": False,
...
def main():
    ...
    parser.add_argument("--output_dir", default="outputs/normad_interaction_persona_vs_json_agent_qwen25_14b")
